...
export function SuppliersListPage() {
  ...
  return (
    <PageContainer
      title="Suppliers"
      description="Browse approved supplier catalogs"
    >
      {suppliers && suppliers.length > 0 ? (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {suppliers.map((supplier) => (
            ...
            >
              ...
              <p className="mt-2 text-sm text-gray-600">Click to browse products</p>
            </button>
          ))}
        </div>
      ) : (
        <EmptyState
          ...
          description="No approved suppliers found. Contact support to connect with suppliers."
        />
      )}
    </PageContainer>
  );
