...
let RolesGuard = RolesGuard_1 = class RolesGuard {
    ...
    canActivate(context) {
        ...
        if (!userRole) {
            this.logger.warn(`RolesGuard: No role found for user ${userId} in org ${orgId}`);
            throw new common_1.ForbiddenException('Insufficient permissions');
        }
        const hasRequiredRole = requiredRoles.includes(userRole);
        if (!hasRequiredRole) {
            this.logger.warn(`RolesGuard: User ${userId} with role ${userRole} attempted to access endpoint requiring roles: ${requiredRoles.join(', ')}`);
